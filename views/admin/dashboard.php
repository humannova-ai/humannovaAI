...
// Quiz stats par utilisateur - compte les participations aux événements de type quiz
$stmt = $db->query("SELECT u.id, u.nom, u.prenom, u.email, COUNT(p.id) as total_quiz 
                    FROM utilisateurs u 
                    INNER JOIN participations p ON u.id = p.utilisateur_id 
                    INNER JOIN evenements e ON p.evenement_id = e.id 
                    WHERE e.type = 'quiz' AND p.statut = 'approuve'
                    GROUP BY u.id 
...
$stmt = $db->query("SELECT p.id, u.nom, u.prenom, u.email, e.titre, e.type, p.date_participation 
                    FROM participations p 
                    INNER JOIN utilisateurs u ON p.utilisateur_id = u.id 
                    INNER JOIN evenements e ON p.evenement_id = e.id 
                    WHERE p.statut = 'en_attente' 
                    ORDER BY p.date_participation DESC LIMIT 5");
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Pending count badge */
        .pending-count {
            background: linear-gradient(135deg, var(--accent-orange), #c77a00);
            color: #000;
            padding: 6px 14px;
            border-radius: 20px;
            font-size: 14px;
