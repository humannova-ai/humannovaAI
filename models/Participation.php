...
class Participation {
    ...
    /**
     * Mettre à jour uniquement le statut (sans toucher aux autres champs)
     * Pour l'admin qui ne doit pas modifier le commentaire ou le fichier
     * @return bool
     */
    public function updateStatutOnly() {
        $query = "UPDATE " . $this->table . " SET statut = :statut WHERE id = :id";
        $stmt = $this->conn->prepare($query);
        $id = $this->getId();
        $statut = $this->getStatut();
