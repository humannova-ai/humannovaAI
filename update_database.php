...
try {
    ...
    // Ajouter la colonne banned si elle n'existe pas
    $checkColumn = $conn->query("SHOW COLUMNS FROM user LIKE 'banned'");
    if ($checkColumn->rowCount() == 0) {
        $conn->exec("ALTER TABLE user ADD COLUMN banned TINYINT(1) DEFAULT 0");
        echo "Colonne 'banned' ajoutée avec succès.\n";
    } else {
        echo "La colonne 'banned' existe déjà.\n";
    }
