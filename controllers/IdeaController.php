...
class IdeaController {
    ...
    public function getAllUsers() {
        try {
            $stmt = $this->pdo->query("SELECT id, fullname, username, email FROM utilisateurs ORDER BY fullname");
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
    ...
    /**
     * Send notification email to all users when a new idea is added
     * @param array $ideaData The idea data (titre, description, etc.)
     * @return void
     */
    private function sendNotificationEmail($ideaData) {
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'najjaryomna12@gmail.com';
            $mail->Password = 'rjjh putq wjsr njpn';
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port = 587;
            
            // Sender
            $mail->setFrom('najjaryomna12@gmail.com', 'Pro Manage AI');
            
            // Email content
            $mail->isHTML(true);
            $mail->Subject = 'New Innovation Idea Added - Pro Manage AI';
            $mail->Body = "
                <html>
                <head>
                    <style>
                        ...
                        .header { background: linear-gradient(135deg, #9945ff, #00ffff); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0; }
                        .content { background: #f9f9f9; padding: 20px; border-radius: 0 0 10px 10px; }
                        .idea-title { color: #9945ff; font-size: 18px; font-weight: bold; margin-bottom: 10px; }
                        .idea-description { color: #333; line-height: 1.6; }
                ...
                <body>
                    <div class='container'>
                        <div class='header'>
                            <h1>💡 New Innovation Idea</h1>
                            <p>A new idea has been added to the platform!</p>
                        </div>
                        <div class='content'>
                            <div class='idea-title'>{$ideaData['titre']}</div>
                            <div class='idea-description'>" . nl2br(htmlspecialchars(substr($ideaData['description'], 0, 200))) . (strlen($ideaData['description']) > 200 ? '...' : '') . "</div>
                            <p><strong>Stay tuned for more updates!</strong></p>
                        </div>
                        <div class='footer'>
                            <p>This is an automated notification from Pro Manage AI</p>
                        </div>
                    </div>
                </body>
                </html>
            ...
            // Add all users as BCC recipients
            ...
            // Log the error but don't fail the idea creation
